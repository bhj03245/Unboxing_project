...
<?php
    ...
    if($page!=1){ //첫 페이지가 아니면 처음 버튼 생성
        $paging .= '&ensp;<a href="./normList.php?page=1">&laquo;</a>';
    }

    if($currentBlock!=1){ //첫 블럭이 아니면 이전 버튼 생성
        $paging .= '&ensp;<a href="./normList.php?page=' . $prevPage . '"><</a>';
    }
    ...
    for($i=$firstPage; $i<=$lastPage; $i++){
        if($i==$page){
            ...
            $paging .= '&ensp;<a href="./normList.php?page='.$i.'">'.$i.'</a>';
        }
    }

    if($currentBlock!=$allBlock){ //마지막 블럭이 아니면 다음 버튼 생성
        $paging .= '&ensp;<a href="./normList.php?page='.$nextPage.'">></a>';
    }

    if($page!=$allPage){
        $paging .= '&ensp;<a href="./normList.php?page='.$allPage.'">&raquo;</a>';
    }
    $paging .= '</ul>';
